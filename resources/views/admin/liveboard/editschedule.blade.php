...
margin-bottom: 8px!important;">
    ...
    <div class="card" id="activty">
        ...
        <div class="card-body p-lg-20">
            ...
            <div class="card-body p-lg-20">
                <label class="form-label">  Liveboard </label>
                <select v-model.number="activityId"  @change = "getimages" class="form-control form-control-solid" aria-label="Disabled select example">
                    ...
                    @foreach (App\Models\LiveBoard::all() as $type)
                    ...
                    <form id='newblog'>
                        ...
                        <div class="row m-3">
                            ...
                            <button class="m-5 btn btn-primary"  @click="getimages" type="button">   Update Schedule   </button>
                    </form>
                

                <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
                    <!--begin::Col-->
                        <div class="col-sm-6 col-lg-4" v-for='date2 in dates'>
                            <!--begin::Card widget 14-->
                            <div class="card card-flush h-xl-100">
                                <label class="form-label">    from  </label>
                                <input  class="form-control" name="from" type="date" :value="date2.from">
                                <label class="form-label">    to  </label>
                                <input  class="form-control" name="to" type="date" :value="date2.to">
                                <br>
                                <div class="card-footer d-flex flex-stack pt-0"> 
                                    <button type="button" @click="deleteFunction(date2.id)" class="btn btn-sm btn-danger flex-shrink-0">Delete</button>
                                </div>
                            </div>
                        </div>
                </div>
            ...
            app = new Vue({
                ...
                methods:{
                    ...
                    getimages:function()
                    {
                        ...
                        url='{{route('live.getschedulevue',':id')}}';
                        url=url.replace(':id',this.activityId);
                        axios.get(url).then(response => {
                            console.log(response);
                            this.dates=response.data.liveboardschedule;
                ...
                savedata:function(){
                    this.error = []
                    this.validation(this.activityId , ' اختر النشاط  ')
                    ...
                    axios.post('{{ route('live.updateschedule') }}', formData).then(response => {
